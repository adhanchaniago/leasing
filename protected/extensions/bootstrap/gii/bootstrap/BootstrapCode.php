...
class BootstrapCode extends CrudCode
{
    public function generateControlGroup($modelClass, $column)
    {
        if ($column->type === 'boolean') {
            ...
            if (stripos($column->dbType, 'text') !== false) {
                return "TbHtml::activeTextAreaControlGroup(\$model,'{$column->name}',array('class'=>'form-control','rows'=>6,'span'=>8))";
            } else {
    ...
    public function generateActiveControlGroup($modelClass, $column)
    {
        if ($column->type === 'boolean') {
            ...
            if (stripos($column->dbType, 'text') !== false) {
                return "\$form->textAreaControlGroup(\$model,'{$column->name}',array('class'=>'form-control','rows'=>6,'span'=>8))";
            } else {
                if (preg_match('/^(password|pass|passwd|passcode)$/i', $column->name)) {
